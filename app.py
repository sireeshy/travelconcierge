...
# Both app variants (this one and the strip layout) write usage and feedback into the same shared
# Sheets (see HANDOFF.md) so every row is tagged with which one it came from -- otherwise "which
# layout gets used more, and rated better" is unanswerable.
...
def _haversine_km(lat1: float, lng1: float, lat2: float, lng2: float) -> float:
    """Great-circle distance in km -- used below to match a route's polyline against known toll
    plaza coordinates (a plaza sits directly on the highway, so a route using that highway passes
    very close to it)."""
    r = 6371.0
...
def _route_cumulative_km(path: list[tuple[float, float]]) -> list[float]:
    """Cumulative distance in km at each point of a decoded route polyline -- cum[i] is how far
    along the route (from the start) that point is. Used below so a candidate place's position can
    be expressed as a real, computed "X km into the trip" instead of the model guessing from city
    names, which is exactly the gap that let search_places_along_route's own candidate list quietly
    cluster near the origin without anyone -- model or code -- noticing."""
    cum = [0.0]
...
def _distance_along_route_km(path: list[tuple[float, float]], cum: list[float], lat: float, lng: float) -> float:
    """How far along the route (in km from the start) the nearest point on the polyline to (lat,
    lng) is -- nearest-vertex matching, not a precise point-to-segment projection, but accurate
    enough at the polyline's point density for both toll-plaza matching and place selection."""
...
def calculate_route_and_etas(origin: str, destination: str, departure_time_iso: str) -> dict:
    ...
    route_cum = _route_cumulative_km(route_path)
    ...
    # distance-only guess (see its _break_points) rather than the whole plan failing over a
...
def search_places_along_route(categories: list[str]) -> dict:
    ...
    for category, point_results in by_category.items():
        ...
        for km, places_data, error, source in point_results:
            ...
            lat, lng = loc.get('latitude'), loc.get('longitude')
            precise_km = _distance_along_route_km(route_path, route_cum, lat, lng) if lat is not None and lng is not None else km
...
preferences_notes = st.text_area(
    ...
    # Mysuru default route below, a genuinely popular weekend-getaway corridor for exactly that.
